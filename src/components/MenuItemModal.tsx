...
export default function MenuItemModal({
  ...
  useEffect(() => {
    ...
    const loadOptions = async () => {
      ...
      const groups = await fetchMenuItemWithOptions(viewItem.id);
  ...
  const handleAddToCart = () => {
    ...
    if (viewItem.is_trackable) {
      const available = Number(viewItem.current_stock ?? 0);
      if (available <= 0) {
        alert("This item is sold out.");
        return;
      }
      if (qty > available) {
        alert(`Only ${available} item(s) left in stock.`);
        return;
      }
    }
