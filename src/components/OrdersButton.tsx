...
const OrdersButton: React.FC<OrdersButtonProps> = ({
  ...
  const [seniorPwdEligible, setSeniorPwdEligible] = useState<boolean | null>(null);
  const [seniorPwdCount, setSeniorPwdCount] = useState(0);
  ...
  const openDiscountDeclaration = () => {
    setSeniorPwdEligible(null);
    setSeniorPwdCount(0);
    setTotalGuests(1);
    setDiscountAmount(0);
  ...
  const handleDiscountDeclarationSubmit = () => {
    setDiscountError(null);

    if (seniorPwdEligible === null) {
      setDiscountError("Please select whether Senior/PWD guests are present.");
      return;
    }

    if (seniorPwdEligible) {
      if (totalGuests < 1) {
        setDiscountError("Please enter the total guest count.");
        return;
      }
      if (seniorPwdCount < 1) {
        setDiscountError("Please enter the number of Senior/PWD guests.");
        return;
      }
      if (seniorPwdCount > totalGuests) {
        setDiscountError("Senior/PWD count cannot exceed total guests.");
        return;
      }

      const perPersonShare = unpaidTotal / totalGuests;
      const discount = perPersonShare * seniorPwdCount * 0.2;
  ...
  const openPaymentFlow = () => {
    ...
    if (computedUnpaidOrders.length === 0) {
      alert("No unpaid orders available for payment.");
      return;
    }

    if (available === 1) {
      const method = cashEnabled ? "cash" : "gcash";
      openDiscountDeclaration();
  ...
  const finalPaymentAmount = useMemo(() => {
    return Math.max(unpaidTotal - discountAmount, 0);
  }, [unpaidTotal, discountAmount]);
    ...
    try {
      ...
      await supabase
        ...
        .update({
          ...
          total_guests: seniorPwdEligible ? totalGuests : 0,
          senior_pwd_count: seniorPwdEligible ? seniorPwdCount : 0,
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-20 right-4 z-[999] w-14 h-14 rounded-full bg-blue-600 text-white shadow-lg flex items-center justify-center hover:bg-blue-700 transition sm:bottom-6 sm:right-6 sm:w-16 sm:h-16"
      >
          ...
          <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="bg-blue-600 px-6 py-4 flex items-center justify-between">
              ...
              <button
                ...
                className="text-white font-bold px-3 py-1 border border-white rounded hover:bg-white hover:text-blue-600"
              >
                Close
              </button>
            </div>
            ...
            <div className="flex border-b">
              <button
                className={`flex-1 py-2 ${
                  activeTab === "cart" ? "font-bold border-b-2 border-blue-600" : "text-gray-500"
                }`}
              ...
              <button
                className={`flex-1 py-2 ${
                  activeTab === "status" ? "font-bold border-b-2 border-blue-600" : "text-gray-500"
                }`}
              ...
              <button
                className={`flex-1 py-2 ${
                  activeTab === "receipts" ? "font-bold border-b-2 border-blue-600" : "text-gray-500"
                }`}
                    ...
                    <>
                      ...
                      <button
                        ...
                        className={`mt-3 w-full py-2 rounded-md text-white ${
                          isDineIn ? "bg-blue-600 hover:bg-blue-700" : "bg-gray-400"
                        }`}
                      >
                        {submittingOrder
                          ? "Submitting..."
                ...
                <div className="space-y-3">
                  ...
                  {currentOrder ? (
                    <div className="p-4 border rounded-lg bg-gray-50">
                      <p className="font-medium text-blue-600 text-lg capitalize">
        ...
        <div className="fixed inset-0 z-[1100] bg-slate-950/20 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-sm bg-white rounded-3xl shadow-2xl p-6 animate-scaleIn">
            <div className="text-center mb-4">
              <h3 className="text-xl font-bold text-gray-800">Senior / PWD Discount</h3>
              <p className="text-sm text-gray-500 mt-2">
                Please declare whether there are Senior or PWD guests before continuing to payment.
              </p>
            </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <button
                  type="button"
                  onClick={() => setSeniorPwdEligible(true)}
                  className={`py-3 rounded-2xl font-semibold transition ${seniorPwdEligible === true ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}
                >
                  Yes
                </button>
                <button
                  type="button"
                  onClick={() => setSeniorPwdEligible(false)}
                  className={`py-3 rounded-2xl font-semibold transition ${seniorPwdEligible === false ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}
                >
                  No
                </button>
              </div>
              ...
              {seniorPwdEligible && (
                <div className="space-y-3 rounded-2xl border border-gray-200 bg-gray-50 p-4">
                  ...
                  <input
                    ...
                    className="w-full rounded-2xl border border-gray-300 px-4 py-3 focus:ring-blue-500 focus:border-blue-500"
                  />

                  <label className="block text-sm font-medium text-gray-700">
                    Senior / PWD guest count
                  </label>
                  <input
                    type="number"
                    min={1}
                    max={totalGuests}
                    value={seniorPwdCount}
                    onChange={(event) => setSeniorPwdCount(Number(event.target.value))}
                    className="w-full rounded-2xl border border-gray-300 px-4 py-3 focus:ring-blue-500 focus:border-blue-500"
                  />
                </div>
              )}
              ...
              <div className="bg-gray-50 border rounded-2xl p-4 text-center">
                ...
                {discountAmount > 0 && (
                  <p className="text-xs text-green-700 mt-2">
                    Discount applied: -₱{discountAmount.toFixed(2)} for {seniorPwdCount} Senior/PWD guest(s)
                  </p>
                )}
              </div>
