...
export async function PATCH(req: Request, { params }: { params: { staffId: string } }) {
  ...
  if (Object.keys(updatePayload).length > 0) {
    ...
    if (updateError) {
      return createJsonError("Unable to update staff account", 500, updateError.message);
    }
  }
  ...
  if (inputPermissions) {
    ...
    if (deleteError) {
      return createJsonError("Unable to clear existing permissions", 500, deleteError.message);
    }

    const permissionRows = inputPermissions.map((permission: any) => ({
      staff_id: staffId,
    ...
    if (permError) {
      return createJsonError("Unable to update permissions", 500, permError.message);
    }
  }
...
export async function DELETE(req: Request, { params }: { params: { staffId: string } }) {
  ...
  if (error) {
    return new NextResponse("Unable to disable staff account", { status: 500 });
  }
